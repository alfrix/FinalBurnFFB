...
static INLINE unsigned int CalcCol(unsigned short *ps,int i)
{
...
int TmntPalUpdate(int bRecalc)
{
  ...
  if (!bAnyDirt) return 0; // All the same

  ps=(unsigned short *)TmntRam08; ph=TmntPal; pd=Dirt;
  for (a=0;a<0x80; a++,pd++)
  {
    int b; unsigned char d; unsigned char m; // dirt info and mask
    d=*pd;
    if (d==0) { ps+=16; ph+=8; continue; } // Not changed
    
    for (b=0,m=1; b<8; b++,m<<=1, ps+=2,ph++)
    {
      if (d&m) *ph=CalcCol(ps,(a<<3)|b); // If dirty recalculate
    }
...
{
  a^=1; a&=0xfff; // Clip to palette length
  if (TmntRam08[a]==d) return; // No change
  TmntRam08[a]=d; // write byte

  a>>=2; Dirt[a>>3]|=1<<(a&7);  bAnyDirt=1; // Mark this color as changed
}
