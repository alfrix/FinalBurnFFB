...
static int JoystickState(DIJOYSTATE *pState,int SubCode)
{
  if (SubCode<5)
  {
    ...
    switch (SubCode)
    {
      case 1: return pState->lY<-0x08000;
      case 2: return pState->lY> 0x08000;
      case 3: return pState->lX<-0x08000;
      case 4: return pState->lX> 0x08000;
    }
    return 0;
  }
...
int DirInputAxis(int i,int Axis)
{
  ...
  if (i<0 || i>=JoystickCount) return 0; // This joystick number isn't connected
  if (ReadJoystick(i)!=0) return 0; // There was an error polling the joystick
